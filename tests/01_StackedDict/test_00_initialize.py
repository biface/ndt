...
def stacked_result(standard_strict_setup):
    return _StackedDict(zip(['first', 'second', 'third', 'fourth'],
...
class TestStackedDictParameters:
    ...
    def test_param_smooth_defaults_setup(self, smooth_c_sd):
        ...
        assert smooth_c_sd.default_setup == [("indent", 2), ("default_factory", _StackedDict)]
...
class TestStackedDictInit:
    ...
    def test_init_zip(self, standard_strict_setup, stacked_result):
        sd = _StackedDict(zip(['first', 'second', 'third', 'fourth'],
                              [1, {'1': "2:1", '2': "2:2", '3': "3:2"}, 3, 4]),
                          default_setup=standard_strict_setup)
        assert sd.indent == 2
        assert sd.default_factory is None
    ...
    def test_init_hybrid(self, standard_strict_setup, stacked_result):
        sd = _StackedDict([('first', 1), ('fourth', 4)],
                          third=3,
                          second={'1': "2:1", '2': "2:2", '3': "3:2"},
                          default_setup=standard_strict_setup)
        assert sd.indent == 2
...
class TestStackedDictErrors:
    ...
    @pytest.mark.parametrize("parameters, error, error_msg", [
        ({}, StackedKeyError, "Missing 'indent' argument in default settings (key: indent)"),
        ({"indent": 0}, StackedKeyError, "Missing 'default_factory' argument in default settings (key: default_factory)"),
        ({"default_factory": None}, StackedKeyError, "Missing 'indent' argument in default settings (key: indent)"),
        ({"indent": 0, "default_factory": None, "balanced_factory": True}, StackedAttributeError,
