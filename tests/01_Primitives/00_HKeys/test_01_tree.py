...
class TestHKeyTree:
    ...
    def test_init(self, key_tree):
        ...
        assert key_tree.get_statistics() == {'total_nodes': 113, 'leaf_count': 74, 'max_depth': 5,
                                             'avg_branching_factor': 2.87, 'total_paths': 112, 'levels': 5}

    def test_balance(self, key_tree):
        assert not key_tree.is_balanced()
        assert key_tree.get_balance_factor() != 0

    @pytest.mark.parametrize("keys, children", [
        ([('env', 'production')], 2),
    ...
    @pytest.mark.parametrize("path, depth, max_depth", [
        (None, 0, 5),
        ([('env', 'dev')], 0, 4),
    ...
    @pytest.mark.parametrize("parent_path, child_key", [
    ...
    def test_get_descendants(self, key_tree, parent_path, child_key):
        assert child_key in [child.key for child in key_tree.find_by_path(parent_path).get_descendants()]


    @pytest.mark.parametrize("key, children", [
        ('global_settings', 3),
        (('env', 'production'), 2),
        ('monitoring', 3)
    ])
    ...
    def test_get_simple_children_by_key_failed(self, key_tree):
        with pytest.raises(TypeError, match=re.escape("_HKey.find_by_key() missing 1 required positional argument: 'key'")):
            key_tree.find_by_key()

    @pytest.mark.parametrize("key, children", [
        (('env', 'dev'), [3, 3, 3, 0, 3]),
        (('env', 'production'), [2, 3, 3, 0, 3])
    ])
    def test_get_multiple_children_by_key(self, key_tree, key, children):
        for n_child in children:
            assert n_child in [len(child.children) for child in key_tree.find_by_key(key, find_all=True)]
        assert children == [len(child.children) for child in key_tree.find_by_key(key, find_all=True)]