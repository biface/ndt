...
from ndict_tools.exception import StackedDictionaryError, NestedDictionaryException


@pytest.mark.parametrize("msg, code, path, result", [
    ("Test error message", 0, [], ["Test error message", 0, []]),
    ("Another test error message", 42, [], ["Another test error message", 42, []]),
    ("Test error message with path", 54, ['path', 'to', 'key'],
     ["Test error message with path (at path: path | to | key)", 54, ["path", "to", "key"]]),
    ("", 0, ["path", "to", "key"],
